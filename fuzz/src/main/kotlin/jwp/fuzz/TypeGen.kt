...
object TypeGen {
    ...
    val interestingShort by lazy {
        interestingByte cat listOf(-32768, -129, 128, 255, 256, 512, 1000, 1024, 4096, 32767)
    }
    val interestingInt by lazy {
        interestingShort cat listOf(Int.MIN_VALUE, -100663046, -32769, 32768, 65535, 65536, 100663045, Int.MAX_VALUE)
    }
    val interestingLong by lazy {
        interestingInt.map { it.toLong() } cat listOf(Long.MIN_VALUE, Long.MAX_VALUE)
    }
    ...
    val interestingDouble by lazy {
        interestingFloat.map { it.toDouble() } cat
    ...
    val suggestedShort by lazy { suggestedIntSimpleRange cat interestingShort }
    val suggestedInt by lazy { suggestedIntSimpleRange cat interestingInt }
    val suggestedLong by lazy { suggestedIntSimpleRange.map { it.toLong() } cat interestingLong }
    val suggestedFloat by lazy { suggestedFloatSimpleRange cat interestingFloat }
    val suggestedDouble by lazy { suggestedFloatSimpleRange.map { it.toDouble() } cat interestingDouble }
